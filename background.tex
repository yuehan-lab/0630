...
on a 50-task Android benchmark; CogAgent~\cite{cogagent2024} achieves state-of-the-art
GUI navigation via a dual-resolution vision-language model without requiring system APIs;
and Mobile-Agent~\cite{mobileagent2024} enables vision-only phone operation without XML
metadata.%
AndroidWorld~\cite{androidworld2024} benchmarks 116 programmatic tasks across 20 real
Android apps, providing a standardized measure of agent capability that underscores the
maturity of the threat.%
Documented fraud use cases include mass account creation, advertising click fraud,
welfare and bonus farming, and silent payment confirmation during remote-assistance
scams~\cite{clickfraud2021,adffraud2021}.%
...
Behavioral biometric systems for mobile authentication---including multi-modal
continuous authentication~\cite{mmauth2022} and few-shot behavioral verification~\cite{authentisense2023}---face
the same spoofability: they measure how interaction \textit{looks}, not whether it
\textit{physically occurred}.%
...
Android's zero-permission motion sensors are already known to leak sensitive private
information: AccEar~\cite{accear2022} reconstructs unconstrained speech from accelerometer
readings of a phone loudspeaker, and StealthyIMU~\cite{stealthyimu2023} infers private
voice-assistant responses from motion sensors without any permission.%
These attacks confirm that zero-permission sensors carry rich physical information about
on-device events---a property we exploit for a defensive purpose.%

\texttt{TYPE\_LINEAR\_ACCELERATION} reports the
